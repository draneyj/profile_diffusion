...
def main() -> None:
    ...
    if not species_info or "masses" not in species_info:
        raise ValueError("Dataset metadata missing species masses")
    ...
    dl_val = None
    if val_idx is not None and inputs_val is not None and int(inputs_val.shape[0]) > 0:
        if loss_mask is not None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        avg_train = running_train / max(1, len(dl_train))
        ...
        if dl_val is not None:
            ...
            avg_val = running_val / max(1, len(dl_val))
            model.train()
